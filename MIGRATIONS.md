...
The plugin `eslint-plugin-n` was renamed from `node-plugin` to `n` to align with the official naming.

Please update references to it in your ESLint configuration and any disabled rules' comments in your codebase.

### Parser changes

`@babel/eslint-parser` was removed and is no longer included by default, resorting to ESLint's default parser instead.

If you rely on Babel-specific syntax, please install `@babel/eslint-parser` and configure it as your parser in your ESLint configuration.
...
`eslint-plugin-mocha` was moved from dependencies to `peerDependencies`, as we now export test framework configs for `jest`, `mocha`, and `vitest` individually. See [Test configs](README.md#test-configs).
Please ensure you have it installed in your project if you use the Mocha test framework, while also importing the Mocha config from `eslint-config-uphold`.

### ESM

The package is now ESM-only. Please ensure your ESLint configuration file is using ESM syntax (e.g. `eslint.config.mjs`).

### `eslint-config-uphold/configs`

If you were using `eslint-config-uphold/configs` to import test framework configs, please ensure you are importing from `eslint-config-uphold` directly instead, as the `configs` entry point was removed.
...
- Some rules were deprecated in ESLint v9. Please run ESLint config inspector to identify any deprecated rules in your config:
